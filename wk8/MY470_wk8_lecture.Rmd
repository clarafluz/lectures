...
-   Open source programming language under [GPL](https://www.gnu.org/licenses/gpl.html)
-   Inspired by the programming language [S](https://en.wikipedia.org/wiki/S_(programming_language))
...
```{r, message = FALSE}
library(pryr)
x <- c(3, 6, 9) #"object with some stuff in it"
x
y <- x

# x and y are going to be stored in the same place!
address(x)
address(y)
```
...
```{r, message = FALSE}
# changing second element of x and no re-assigning of x to y
x[2] <- 4
x

# new version of x has been created, y is now pointing to position of old x
address(x)
address(y)
```
...
rm(x) # can remove specific objects from environment
ls() # shows everything in environment
```
...
firstletters[1:4] # string is a one-dimensional vector, cannot be sliced! -> this will give an error
```
...
```{r}
char_vec[logical_vec] # will return only those where we have a true statement 
```
...
x <- c(1, 2, 3)
y <- c(5, 10)
x * y # what it does: 1*5, 2*10, 3*5 (replicates element to have another one to multiply with)
```

```{r}
x <- 1:20

x * c(1, 0)    # turns the even numbers to 0 -> replicates c over and over to have enough numbers to multiply with
```
...
-   **Using [[ -\> will use this more often!**

```{r}
my_list[["something"]] # get back directly the vector that belongs to it!

my_list[[1]] # get back actual numeric vector of interest rather than accessing first element of list
...
-   **Using \$ -\> will give back direct data but only works with named list and cannot be done with multiple elements to be indexed in one command**
...
-   but think about what it does -\> saves a new version missing these elements
...
1.  Introduction
2.  Fundamentals and data structures
3.  **Control flow -\> very similar to Python but no indentation (instead: {})**
4.  Functions
...
-   everything in R is basically a vector and we can do all kinds of operations on a vector which is much more efficient
...
# vs:
z <- x*y # uses matrix algebra to do the same essentially
...
```{r}
my_addition_function <- function(a = 10, b) { # default values defined here
    return(a + b)
}

my_addition_function(a = 5, b = 50)
my_addition_function(3, 4) # need to get the order right!, if we assign it, order doesn't matter
...
a <- 1
my_demo_function(a = 20) # will not pay attention to global workspace, but only because we provided it inside the function bracket
a
```
...
x %>% mean() # uses previous level as input to this function
```
...
2.  "Pure" functions: The output only depends on the inputs, i.e. if you call it again with the same inputs, you get the same outputs. The function also has no side-effects, like changing the value of a global variable, writing to disk, or displaying to the screen. So, e.g., `y <- 4; my_function <- function(x) {return(y + x)}` is not a pure function (because y is an input that will change the output of the function).\
    -\> want to use this as much as possible
